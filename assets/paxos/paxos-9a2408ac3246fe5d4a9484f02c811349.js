...
(function() {
  ...
  Harry.NetworkMember = {
    sendMessage: function(destinationID, message) {
      return this.network.sendMessage(this.id, destinationID, message);
    },
    broadcastMessage: function(message) {
      return this.network.broadcastMessage(this.id, message);
    },
    replyTimeout: 8000,
...
(function() {
  ...
  Harry.Client = (function(_super) {
    ...
    Client.prototype.propose = function() {
      this.nextValue += 10;
      return this.sendMessage(this.replicaIDForMessages(), new Harry.SetValueMessage(this.nextValue));
    };

    Client.prototype.replicaIDForMessages = function() {
      return Math.floor(Math.random() * this.network.replicas.length) + 1;
    };
...
(function() {
  ...
  Harry.PrepareMessage = (function(_super) {
    ...
    function PrepareMessage(sequenceNumber) {
  ...
  Harry.PromiseMessage = (function(_super) {
    ...
    function PromiseMessage() {
      return PromiseMessage.__super__.constructor.apply(this, arguments);
    }
  ...
  Harry.QueryResponseMessage = (function(_super) {
    ...
    function QueryResponseMessage(sequenceNumber, value) {
      this.sequenceNumber = sequenceNumber;
      this.value = value;
    }
  ...
  Harry.GetValueMessage = (function(_super) {

    __extends(GetValueMessage, _super);

    function GetValueMessage() {
      return GetValueMessage.__super__.constructor.apply(this, arguments);
    }

    GetValueMessage.prototype.type = 10;

    return GetValueMessage;
  ...
  Harry.Network = (function(_super) {
    ...
    function Network(optionsOrReplicaCount) {
      ...
      this.replicas = (function() {
        ...
        for (i = _i = 1, _ref2 = this.replicaCount; 1 <= _ref2 ? _i <= _ref2 : _i >= _ref2; i = 1 <= _ref2 ? ++_i : --_i) {
          _results.push(new Harry.Replica(i, this.quorum, this));
        }
        return _results;
      }).call(this);
    ...
    Network.prototype.sendMessage = function(originID, destinationID, message) {
      ...
      if (this.canSend(originID, destinationID)) {
        ...
        if (!(this.entitiesById[originID] && this.entitiesById[destinationID])) {
          debugger;
        }
        return this._deliverMessageIn(flightTime, message);
      }
    };
    ...
    Network.prototype._deliverMessageIn = function(time, message) {
      var _this = this;
      this.fire('messageSent', message, time);
      return setTimeout(function() {
        return _this.entitiesById[message.destination].processMessage(message);
      }, time);
    };
...
(function() {
  ...
  Harry.NetworkVisualizer = (function() {
    ...
    NetworkVisualizer.messageTypeColor = d3.scale.category10();

    NetworkVisualizer.valueScale = d3.scale.category10();
    ...
    NetworkVisualizer.prototype.drawReplicas = function() {
      var _this = this;
      this.replicaSquares = this.svg.selectAll("rect.replica").data(this.network.replicas).attr("fill", function(replica) {
        return _this.constructor.valueScale(replica.value);
      });
      return this.replicaSquares.enter().append("svg:rect").attr("class", "replica").attr("width", this.replicaWidth).attr("height", this.replicaWidth).attr("x", function(replica) {
        return _this.entityX(replica.id) - (_this.replicaWidth / 2);
      }).attr("y", function(replica) {
        return _this.entityY(replica.id) - (_this.replicaWidth / 2);
      });
    };
    ...
    NetworkVisualizer.prototype.drawReplicaLabels = function() {
      ...
      this.sequenceNumberLabels = this.svg.selectAll("text.sequence-number-label").data(this.network.replicas);
      this.sequenceNumberLabels.text(function(replica) {
        ...
        return _this.entityY(replica.id) - 8;
      }).text(function(replica) {
        return replica.highestSeenSequenceNumber;
      });
      this.stateLabels = this.svg.selectAll("text.state-label").data(this.network.replicas);
      this.stateLabels.text(function(replica) {
      ...
      this.valueLabels = this.svg.selectAll("text.value-label").data(this.network.replicas);
      return this.valueLabels.text(function(replica) {
    ...
    NetworkVisualizer.prototype.drawClients = function() {
      var _this = this;
      this.clientCircles = this.svg.selectAll("circle.client").data(this.network.clients);
      return this.clientCircles.enter().append("svg:circle").attr("fill", "#FF00F0").attr("class", "client").attr("r", 20).attr("cx", function(client) {
        return _this.entityX(client.id);
      }).attr("cy", function(client) {
        return _this.entityY(client.id);
      });
    };

    NetworkVisualizer.prototype.attachMessageHandlers = function() {
      var _this = this;
      return this.network.on('messageSent', function(message, flightTime) {
        _this.inFlightMessages.push(message);
        return _this.svg.selectAll("circle.message").data(_this.inFlightMessages, function(message) {
          return message.id;
        }).enter().append("svg:circle").attr("class", "message").attr("r", 8).attr("cx", function(message) {
          return _this.entityX(message.sender);
        }).attr("cy", function(message) {
          return _this.entityY(message.sender);
        }).attr("fill", function(message) {
          return _this.constructor.messageTypeColor(message.type);
        }).transition().duration(flightTime).attr("cx", function(message) {
          return _this.entityX(message.destination);
        }).attr("cy", function(message) {
          return _this.entityY(message.destination);
        }).remove().each("end", function(message) {
          return _this.inFlightMessages.splice(_this.inFlightMessages.indexOf(message), 1);
        }).ease();
      });
    };
    ...
    NetworkVisualizer.prototype.emitValueChange = function(replica) {
      var orb;
      return orb = this.svg.selectAll("circle.value-change.replica-" + replica.id).data([1]).enter().insert("svg:circle", ":first-child").attr("fill", "#CC0000").attr("class", "value-change replica-" + replica.id).attr("r", 17).attr("opacity", 0.6).attr("cx", this.entityX(replica.id)).attr("cy", this.entityY(replica.id)).transition().duration(1000).attr("r", 40).attr("opacity", 0).remove().ease();
    };
...
(function() {
  ...
  Harry.Replica = (function(_super) {
    ...
    Replica.prototype.setValue = function(value, callback) {
      this.set('highestSeenSequenceNumber', this.get('highestSeenSequenceNumber') + 1);
      this.round = new Harry.Round({
        sequenceNumber: this.get('highestSeenSequenceNumber'),
        callback: callback,
        value: value,
        promisesReceived: 0
      });
      return this.startTransition('startSet');
    };

    Replica.prototype.getValue = function() {};

    Replica.prototype.processMessage = function(message) {
      switch (this.get('state')) {
        case 'awaiting-promises':
    ...
    Replica.prototype.on('startSet', function() {
      var _this = this;
      this.broadcastMessage(new Harry.PrepareMessage(this.round.sequenceNumber));
    ...
    Replica.prototype.on('proposalSucceeded', function() {
      var round;
      this.broadcastMessage(new Harry.AcceptMessage(this.round.sequenceNumber, this.round.value));
      this.set('value', this.round.value);
      round = this.round;
      delete this.round;
      return typeof round.callback === "function" ? round.callback() : void 0;
    });
    ...
    Replica.prototype.on('proposalFailed', function() {
      ...
      round = this.round;
      delete this.round;
      return typeof round.callback === "function" ? round.callback(new Error("value not written")) : void 0;
    });
    ...
    Replica.prototype.promiseReceived = function() {
      this.round.promisesReceived += 1;
      if (this.round.promisesReceived >= this.quorum) {
    ...
    Replica.prototype.prepareReceived = function(message) {
      var response;
      response = message.sequenceNumber > this.get('highestSeenSequenceNumber') ? (this.set('highestSeenSequenceNumber', message.sequenceNumber), new Harry.PromiseMessage()) : new Harry.RejectProposalMessage(this.get('highestSeenSequenceNumber'));
      return this.sendMessage(message.sender, response);
    };
...
(function() {
  var acceptVisualization, clientOnlyVisualization, mainVisualization, prepareOnlyVisualization, promiseVisualization;
  ...
  prepareOnlyVisualization = new Harry.NetworkVisualizer({
    ...
    onStart: function(visualization, network) {
      ...
      return firstReplica.on('startSet', function() {
